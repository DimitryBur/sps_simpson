...
1.  Compute squared row norms: \(n_i = \|A_{i,:}\|_2^2\)
2.  Calculate probabilities: \(p_i = n_i / \sum_j n_j\)
3.  Simpson index: \( \text{Simpson} = 1 - \sum_i p_i^2 \)
4.  Effective rows: \(m_{\text{eff}} = 1 / (1 - \text{Simpson})\)
5.  Repeat for columns to get \(n_{\text{eff}}\)
6.  **Effective rank estimate:** \( k_{\text{eff}} = \lfloor \sqrt{m_{\text{eff}} \cdot n_{\text{eff}}} \rfloor \)
